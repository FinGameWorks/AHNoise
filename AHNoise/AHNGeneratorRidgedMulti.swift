...
///Generates cohesive ridges, useful as height maps for mountainous regions. The noise created lies within the range -1.0 - 1.0 numerically [0.0 - 1.0 in colour space].
///
///*Conforms to the `AHNTextureProvider` protocol.*
public class AHNGeneratorRidgedMulti: AHNGenerator {
  ...
  /**
   Creates a new `AHNGeneratorRidgedMulti` object.
  ...
  public init(context: AHNContext, textureWidth width: Int, textureHeight height: Int, use4DNoise: Bool, mapForSphere: Bool, makeSeamless: Bool){
    super.init(functionName: "ridgedMultiGenerator", context: context, textureWidth: width, textureHeight: height, use4DNoise: use4DNoise, mapForSphere: mapForSphere, makeSeamless: makeSeamless)
  }
